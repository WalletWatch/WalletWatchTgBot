...
def make_token_keyboard(wallet_id):
    ...
    for i in wallets:
        name = i[1]
        inline_keyboard.add(types.InlineKeyboardButton(text=f"{name}", callback_data=str(i[0])))
...
def make_token_menu(token_id, track):
    ...
    builder.add(types.InlineKeyboardButton(text=text_track, callback_data=str(token_id)))
