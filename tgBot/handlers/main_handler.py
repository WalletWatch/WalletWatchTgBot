...
from text import GREETING, INFO, INFO_COMPACT
from kb import make_kb_main, make_kb_wallets, make_kb_tokens
...
@router_main.callback_query(F.data == "walletmenu")
async def wallet_menu_hendler(callback: CallbackQuery):
    await callback.message.answer("Выберите команду:", reply_markup=make_kb_wallets())

@router_main.callback_query(F.data == "tokenmenu")
async def token_menu_hendler(callback: CallbackQuery):
    await callback.message.answer("Выберите команду:", reply_markup=make_kb_tokens())
